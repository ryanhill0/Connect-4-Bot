...
difficulty_1 = int(input("Select Player 1 difficulty: 1 - Easy, 2 - Medium, 3 - Hard, 4 - User: "))
difficulty_2 = int(input("Select Player 2 difficulty: 1 - Easy, 2 - Medium, 3 - Hard, 4 - User: "))
...
def pickBestMove(board,piece):
	...
	for c in valid_locations:
		...
		score = window_score(temp_board,piece,medHeuristic)
		if score > best_score:
			best_score = score
			best_col = c
	return best_col
...
def minimax(board,depth,maxplayer,heuristic):
	...
	if depth ==0 or isTerminal:
		if isTerminal:
			...
			return None, window_score(board, AI_PIECE,medHeuristic)
	if maxplayer:
		value = -math.inf
...
while not game_over:
	...
	if turn == PLAYER and not game_over:
		...
		if diff_tup[0] == HARD:
			col, minimax_score = abminimax(board,6,-math.inf, math.inf, True, hardHeuristic)
			if is_valid_location(board, col):
				pygame.time.wait(500)
				row = get_next_open_row(board, col)
				drop_piece(board, row, col, PLAYER_PIECE)
	...
	if turn == AI and not game_over:
		...
		if diff_tup[1] == HARD:
            #implementing hard abminmax with heuristic
			col, minimax_score = abminimax(board,6,-math.inf, math.inf, True, hardHeuristic)
